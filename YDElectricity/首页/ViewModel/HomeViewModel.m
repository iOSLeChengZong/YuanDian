...
//分类商品
-(void)getCommodityModelDataRequestMode:(RequestMode)requestMode requestWord:(NSString *)requestW requestPram:(NSString *)pram classID:(NSString *)ID pageSize:(NSInteger)pageSize CompletionHandler:(void (^)(NSError * _Nonnull))completionHandler{
    NSInteger tempNum = 1;
    if (requestMode == RequestModeMore) {
        tempNum = ++_pageNum;
    }
    [YDNetManager getCategoryCommodityListDataWithPath:kCommodityDetailURL requestWord:requestW requestPram:pram classID:ID pageNum:tempNum goodNum:pageSize completionHandler:^(HomeGoodModel * _Nonnull model, NSError * _Nonnull error) {
        if ((!error)) {
            _pageNum = tempNum;
            if (requestMode == RequestModeRefresh) {
                [self.pageList removeAllObjects];
